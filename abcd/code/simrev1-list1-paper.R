...
# Analysis of list 1
rm(list = ls())
setwd("~/Documents/GitHub/disparity-sensitivity")
...
generatePlot <- function(num_cov_lbl = 8, num_pt_lbl = 3, psize = 6, estimand = "reduction") {
  # plot type: point_est or confidence
  ...
  p1 <- df_plot %>%
    ...
    metR::geom_contour_fill(breaks = c(maxbias, 1000 * maxbias), fill = 'dodgerblue3', 
                            alpha = switch(estimand, "reduction" = 0.2, "residual" = 0.26)) +
