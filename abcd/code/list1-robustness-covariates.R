...
df_yz <- df_yz %>% mutate(w_rmpw = w_rmpw,
                          e1 = e1,
                          e0 = e0)
write_csv(df_yz, paste0("../data/list1_robust_YZGW_", Z_method, "_", outcome, ".csv"))
...
e0_obj <- weightit(Z ~ . - by, data = df_e0, method = "ebal") %>% trim(at = 0.9, lower = TRUE)
# by computes propensity scores within each group of by; different from weights in glm()
e1_obj <- weightit(Z ~ . - by,
                   data = df_e1, method = "ebal") %>% trim(at = 0.9, lower = TRUE)
