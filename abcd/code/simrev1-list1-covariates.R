# processing covariates for sim1rev
rm(list = ls())
...
df_yz <- read_csv(paste0("../data/list1_simrev1_YZG_", Z_method, "_", outcome, ".csv"))
...
df_allowable <- model.matrix(~ .^2 -1, data = df_x %>% select(all_of(allowable_covs))) %>% 
  data.frame() %>% NAImpute() %>% tibble() %>% dplyr::select(-sexF)
...
df_yz <- df_yz %>% mutate(w_rmpw = w_rmpw,
                          e1 = e1,
                          e0 = e0)
write_csv(df_yz, paste0("../data/list1_simrev1_YZGW_", Z_method, "_", outcome, ".csv"))
